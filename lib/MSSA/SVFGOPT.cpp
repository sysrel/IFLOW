...
llvm::cl::opt<bool>
InfoFlowTargetInfo("info-flow-target",
                    llvm::cl::desc("Print target resolution info\n"),
                    llvm::cl::init(false));

void printCSResol(SVFG *svfg) {
  PAG *pag = svfg->getPAG();
...
void SVFGOPT::buildSVFG() {
    ...
    if (InfoFlow) {
       ...
       ICFG *icfg = new ICFG(pta->getPTACallGraph());
       icfg->dump("icfg");
       dump("SVFG_after_prop_indcalls"); 

       InfoFlowEngine *ifl = new InfoFlowEngine(this, icfg);
       ifl->runQuery();
       ifl->finalizeStats();
       printCSResol(this);
       exit(0);
...
SVFGEdge* SVFGOPT::addCallIndirectSVFGEdge(NodeID srcId, NodeID dstId, CallSiteID csid, const PointsTo& cpts)
{
    if (ContextInsensitive) {
        //llvm::errs() << "Connnecting indirect SVFG edge between "
...
void SVFGOPT::handleInterValueFlow()
{
    ...
    for (SVFGNodeSet::iterator it = nodesToBeDeleted.begin(), eit = nodesToBeDeleted.end(); it != eit; ++it) {
        ...
        if (canBeRemoved(node)) {
            ...
            llvm::errs() << "Removing node " << node->getId() << "\n";
            removeAllEdges(node);
            removeSVFGNode(node);
        }
    }
...
void SVFGOPT::retargetEdgesOfAInFOut(SVFGNode* node)
{
    ...
    for (; it != eit; ++it) {
        ...
        if (SVFUtil::isa<ActualINSVFGNode>(node)) {
            llvm::errs() << "Redirecting Actual IN node " << node->getId() << "\n";
