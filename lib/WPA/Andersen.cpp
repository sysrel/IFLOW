...
bool Andersen::collapseField(NodeID nodeId)
{
    ...
    /* SYSREL extension */
    /* PAGNode *bp = pag->getPAGNode(baseId);
    const Value *bv = bp->getValue();
    ...
    // SVFUtil::getSourceLoc(bi)
    if (bi) {
       MDNode *md = bi->getMetadata("dbg");
       if (md) {
          llvm::DebugLoc loc(md);
          line = loc.getLine();
          llvm::errs() << "Andersen Field collapse srcinfo," << (*bv) << "," << file << "," << func << "," << line << "\n";
       } 
    }
    else llvm::errs() << (*bp) << "\n"; */
