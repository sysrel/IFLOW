...
bool MTA::runOnModule(SVFModule module) {
     ...
     */
        ...
        FSMPTA *pta = FSMPTA::createFSMPTA(module, mhp,lsa);
        ...
        //    stat->performMHPPairStat(mhp,lsa);
         ...
         // Find synchronization objects
         SVFG::iterator svfgNodeIt = svfg->begin();
         SVFG::iterator svfgNodeEit = svfg->end();
         for (; svfgNodeIt != svfgNodeEit; ++svfgNodeIt)
         {
            SVFGNode* svfgNode = svfgNodeIt->second;
            if (SVFUtil::isa<ActualINSVFGNode>(svfgNode)) 
            {
                ActualINSVFGNode *ain = (ActualINSVFGNode*)svfgNode; 
                CallSite cs = ain->getCallSite();
                const PointsTo& pt = ain->getPointsTo();
                //if (isALockAPI(cs) || isSyncAPI(cs))
                //{
                   const MemSSA::CALLMU* cmu = ain->getCallMU();
                   llvm::errs() << "Instruction: " << (*cs.getInstruction()) ;
                   llvm::errs() << "Points to set: \n";
                   for(PointsTo::iterator it = pt.begin(); it != pt.end(); it++)
                      llvm::errs() << (*it) << "\n";  
                //} 
            }
         }
