...
void SVFGStat::customStateForGep(NodeID id, VFGNode *node) {
    /*GepSVFGNode *gepnode = SVFUtil::dyn_cast<GepSVFGNode>(node);
    if (!gepnode) return;
    PointerAnalysis *pta = getPTA();
    const PointsTo& srcPts = pta->getPts(gepnode->getPAGSrcNodeID());
    ...
    for (PointsTo::iterator piter = srcPts.begin(); piter != srcPts.end(); ++piter) {
        ...
        if (!isBlkObjOrConstantObj(ptd)) {
            if (SVFUtil::isa<VariantGepPE>(edge->getPAGEdge())) {
                if (isObjFieldInsensitive(ptd)) {
                   llvm::errs() << "Varint gep: \n"; 
                } 
            }
        }
    }
...
void SVFGStat::processGraph()
{
    ...
    for (; it != eit; ++it) {
        ...
        if (SVFUtil::isa<FormalINSVFGNode>(it->second))
            ...
            numOfGep++;
            customStateForGep(it->first, it->second);
        }
