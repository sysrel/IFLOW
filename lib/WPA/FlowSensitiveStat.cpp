...
void FlowSensitiveStat::printPtsInfo(PAGNode *pn) {
   if (pn->isPointer()) {
     SVFG* graph = fspta->svfg;
     PAG *pag = fspta->getPAG(); 
...
bool FlowSensitiveStat::violatesSoundness(NodeID q, const CallSite &cs) {
    SVFG* graph = fspta->svfg;
    PAG *pag = fspta->getPAG(); 
        ...
        if (SVFUtil::isa<GepSVFGNode>(it->second)) {
            ...
            if (q == src || q == dst) {
               ...
               if (samePtsSetDiffOffset || emptyPtsToForDst) {
                  ...
                  #ifdef DBGPR
                  llvm::errs() << "dependent flow is part of Gep with identical src-dst poi$
                  #endif
...
void FlowSensitiveStat::getVFGDependencies(const User *u, const CallSite &cs) {
   //llvm::errs() << "Value-flow graph dependencies of " << (*u) << "\n";
   SVFG* graph = fspta->svfg;
   PAG *pag = fspta->getPAG(); 
   PAGNode *pn = pag->getPAGNode(pag->getValueNode(u));
   violatesSoundness(pn->getId(),cs);
   ...
   for (; edgeIt != edgeEit; ++edgeIt) {
       ...
       if (StmtVFGNode *stn = SVFUtil::dyn_cast<StmtVFGNode>(sn)) {
          ...
          if (const User *usucc = SVFUtil::dyn_cast<User>(vs)) {
              getVFGDependencies(usucc,cs);
          }
       }
   }
}

void FlowSensitiveStat::customStat() {
    #ifdef DBGPR 
    llvm::errs() << "SYSREL extension on custom stats\n";
    #endif
                  llvm::errs() << "unresolved callsite,"
                               << "callsite function," 
                               << "callsite file," 
                               << "callsite lineno," 
                               << "gep instruction," 
