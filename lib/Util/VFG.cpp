...
extern llvm::cl::opt<bool> InfoFlow;
...
bool signaturesMatch(CallSite cs, const Function *func) {
  int i=0;
  for(CallSite::arg_iterator itA = cs.arg_begin(), ieA = cs.arg_end(); 
         itA!=ieA; ++itA, i++);
  int j=0; 
  for(Function::const_arg_iterator itA = func->arg_begin(), ieA = func->arg_end();
         itA!=ieA; ++itA, j++);
  if (i != j) return false;
  std::string tstr1;
  llvm::raw_string_ostream rso1(tstr1);
  func->getReturnType()->print(rso1);
  std::string tstr2;
  llvm::raw_string_ostream rso2(tstr2);
  func->getReturnType()->print(rso2);
  if (rso1.str() != rso2.str()) return false;
  int ai=0;
  for(Function::const_arg_iterator itA = func->arg_begin(), ieA = func->arg_end();
         itA!=ieA; ++itA, ai++) {
     std::string atstr1;
     llvm::raw_string_ostream arso1(atstr1);
     cs.getArgument(ai)->getType()->print(arso1);
     std::string atstr2;
     llvm::raw_string_ostream arso2(atstr2);
     (*itA).getType()->print(arso2);
     if (arso1.str() != arso2.str()) return false;   
  }    
  return true;
}
...
void VFG::updateCallGraph(PointerAnalysis* pta)
{
    ...
    for (; iter != eiter; iter++) {
        ...
        for (PointerAnalysis::FunctionSet::const_iterator func_iter = functions.begin(); func_iter != functions.end(); func_iter++) {
            ...
            if (signaturesMatch(newcs, func)) { // SYSREL EXTENSION
               //llvm::errs() << "Connecting caller " << (*newcs.getInstruction()) << " to callee " << func->getName() << "\n"; 
               connectCallerAndCallee(newcs, func, vfEdgesAtIndCallSite);
            }
        }
    }

    // SYSREL extension, we also connect caller callee for direct callsites
    if (!InfoFlow) return;
    PTACallGraph* pcg = pta->getPTACallGraph();
    const llvm::DenseMap<const Instruction*, CallGraphEdgeSet> & direm = pcg->getCallInstToCallGraphEdgesMap();        
    VFGEdgeSetTy vfEdgesAtDirCallSite;
    for(auto me : direm) {
        for(auto ce : me.second) {
           CallSiteID csid = ce->getCallSiteID();
           CallSite cs = pcg->getCallSite(csid);
           const Function *func = SVFUtil::getCallee(cs);
           //llvm::errs() << "csid: " << csid << " function " << func->getName() << "\n";
           if (func) {
              connectCallerAndCallee(cs, func, vfEdgesAtDirCallSite);
           }
        }
    }
}
